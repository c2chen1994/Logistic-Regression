...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    """
    # y label is 0 or 1 not 1 or -1
    y = np.array(y).reshape(N, 1)
    iterations = 1
    while iterations < max_iterations:
        iterations += 1
        a = -sigmoid(np.dot(X, w.reshape(D,1)) + b) + y
        wMean = np.mean(a * X, axis = 0)
        bMean = np.mean(a)
        '''
        #another method
        y = 2 * y - 1
        wG = sigmoid(-np.dot(X, w.reshape(D,1)) * y) * y * X
        bG = sigmoid(-b * y) * y
        wMean = np.mean(wG, axis = 0)
        bMean = np.mean(bG)
        '''
        b += step_size * bMean
        w += step_size * wMean
...
def binary_predict(X, w, b):
    ...
    TODO: add your code here
    """  
    preds = np.round(sigmoid(np.dot(X, w.reshape(D, 1)) + b)).reshape(N,)
...
def multinomial_train(X, y, C, 
    ...
    while iterations < max_iterations:
        iterations += 1
        b_ = np.tile(b, (N,1))
        a = np.dot(X, np.transpose(w)) + b_
        aMax = np.max(a, axis = 1).reshape(N,1)
        a_ = a - np.tile(aMax, (1,C))
        a_exp = np.exp(a_)
        a_expSum = np.sum(a_exp, axis = 1).reshape(N,1)
        p = a_exp / np.tile(a_expSum, (1,C))
        wGSum = np.zeros((C, D))
        bGSum = np.zeros(C)
        for i in range(N):
            p[i][y[i]] -= 1
        bMean = np.sum(p, axis=0) / N
        wMean = np.dot(np.transpose(p), X) / N
        w -= step_size * wMean
        b -= step_size * bMean
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in range(C):
        # if class i then y = 1; if not class i then y = 0
        yNew = np.round(np.exp(-np.abs(y - i)))
        w[i], b[i] = binary_train(X, yNew, w[i], b[i], step_size, max_iterations)
...
def OVR_predict(X, w, b):
    ...
    """
    b_ = np.tile(b, (N,1))
    a = sigmoid(np.dot(X, np.transpose(w)) + b_)
    # the max probability column is the (C - 1) th
    preds = np.argsort(a)[:,C - 1]
...
'''
    ...
    b = np.array([3,7,8,4,6])
    c = np.array([2,1,5,0,4])
    d = a + b
    e = np.array([[1,2,3,4,5],[3,7,8,4,6],[2,1,5,0,4]])
    f_ = np.array([1,2,3,2,1])
    f = np.array(np.matrix(f_).T)
    g = np.dot(e,f)
    h = ([1,0,1])
